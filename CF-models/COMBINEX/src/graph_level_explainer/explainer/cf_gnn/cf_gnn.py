...
class CFExplainer(Explainer):
    ...
    def train(self, graph: Data, oracle: nn.Module) -> Data:
        ...
        y_non_differentiable = torch.argmax(output_actual)
        ...
        if y_non_differentiable == graph.targets and loss.item() < self.best_loss:
            
            counterfactual = build_counterfactual_graph_gc(graph.x, edge_index, graph, oracle, output_actual)
