...
class CFExplainerFeatures(Explainer):
    ...
    def train(self, graph: Data, oracle) -> Data:
        ...
        differentiable_output = self.node_perturber.forward(graph.x, graph.batch) 
        model_out, V_pert, P_x = self.node_perturber.forward_prediction(graph.x, graph.batch) 
        
        y_pred_new_actual = torch.argmax(model_out, dim=1)

        loss, edge_index = self.node_perturber.loss(graph, differentiable_output, y_pred_new_actual)
        loss.backward()
