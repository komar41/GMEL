...
class CFFExplainer(Explainer):
    ...
    def explain(self, graph: Data, oracle) -> dict:
        ...
        for _ in range(self.epochs):
            ...
            out = oracle(graph.x, masked_adj[0], masked_adj[1])
...
class ExplainModelNodeMulti(torch.nn.Module):
    ...
    def forward(self):
        ...
        S_f = self.base_model(self.graph.x, masked_adj[0], masked_adj[1])  # Counterfactual graph
        S_c = self.base_model(self.graph.x, self.graph.edge_index)  # Original graph
        return S_f, S_c
