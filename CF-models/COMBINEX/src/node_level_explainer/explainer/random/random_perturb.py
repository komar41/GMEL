...
class RandomExplainer(Explainer):
    ...
    def explain(self, graph: Data, oracle: nn.Module) -> Data:
        ...
        for _ in range(self.cfg.explainer.epochs):
            ...
            out = oracle(graph.x, graph.edge_index, P_e)
            
            pred_cf = torch.argmax(out, dim=1)            
            
            # Counterfactual fidelity loss (e.g., cross-entropy)
            fidelity_loss = F.cross_entropy(out[graph.new_idx], graph.targets[graph.new_idx])
            ...
            if (pred_cf[graph.new_idx].item() == graph.targets[graph.new_idx].item()) and (total_loss.item() < best_loss):

                counterfactual = build_counterfactual_graph(x=graph.x, edge_index=cf_edge_index, graph=graph, oracle=oracle, output_actual=out)
