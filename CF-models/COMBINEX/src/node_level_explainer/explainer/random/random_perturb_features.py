...
class RandomFeaturesExplainer(Explainer):
    ...
    def explain(self, graph: Data, oracle: nn.Module) -> dict:
        ...
        for _ in range(self.cfg.explainer.epochs):
            ...
            out = oracle(perturbed_features, graph.edge_index)
            ...
            loss_pred = F.cross_entropy(out[graph.new_idx], graph.targets[graph.new_idx])
            ...
            if (pred_cf[graph.new_idx] == graph.targets[graph.new_idx]) and (loss_tot.item() < best_loss):
                counterfactual = build_counterfactual_graph(x=perturbed_features, edge_index=graph.edge_index, graph=graph, oracle=oracle, output_actual=out)
